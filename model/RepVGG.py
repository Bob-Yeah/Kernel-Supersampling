...
def conv_bn(in_channels, out_channels, kernel_size, stride, padding, groups=1):
    ...
    # result.add_module('bn', nn.BatchNorm2d(num_features=out_channels)) # we don't need this
    return result
...
# Rep VGG Feature Extraction
class RepVGGFE(nn.Module): 

    def __init__(self,  deploy=False, use_se=False):
        super(RepVGGFE, self).__init__()
        self.deploy = deploy
        self.use_se = use_se
        ...
        self.stage2 = RepVGGBlock(in_channels=14, out_channels=14, kernel_size=3, stride=1, padding=1, deploy=self.deploy, use_se=self.use_se)
